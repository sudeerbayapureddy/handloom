...
import com.karvy.handloom.common.util.FileUtil;
...
//
		...
		String indentNo = stakeHolderDAO.getMaxNoByColumn(YarnIndent.class, "yarnIndentNumber");
		if (indentNo == null) {
			indent.setYarnIndentNumber(FileUtil.getInitialNumber("indent.InitialNumber"));

		} else {

			int indentNoInInt = Integer.parseInt(indentNo.substring(4));
			indent.setYarnIndentNumber("IND-"+(++indentNoInInt));
		}
		//
	...
	public String submitStakeHolderDetails(StakeHolderBean stakeHolderBean) {
		...
		String regNo = stakeHolderDAO.getMaxNoByColumn(StakeHolder.class, "shRegNo");
		if (regNo == null) {
			stakeHolder.setShRegNo(FileUtil.getInitialNumber("request.InitialNumber"));

		} else {

			int requestInInt = Integer.parseInt(regNo.substring(4));
			stakeHolder.setShRegNo("REQ-"+(++requestInInt));
		}
		//
		...
		for(AncillaryBean bean:ancillaryList){
			...
			for(AncillaryWorkerDetailsBean details:workersDetails){
				...
				worker.setCreated_By(stakeHolderBean.getUname());
				worker.setCreated_Date(new Date());
	...
	@Override
	public String checkRegistrationByUserId(String uname) {
		User user=stakeHolderDAO.checkRegistrationByUserId(uname);
