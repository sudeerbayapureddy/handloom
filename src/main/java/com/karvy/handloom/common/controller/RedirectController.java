...
public class RedirectController {
	...
	public ModelAndView redirect(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
		...
		logger.info("RedirectController redirect {}", httpServletRequest.getRequestURL().toString());

		ModelAndView modelAndView = new ModelAndView("loginLayout", "loginBean", new LoginBean());
		
		if (error != null) {
			if (error.equalsIgnoreCase("1")) {
				logger.info("RedirectController redirect error:Invalid username and password");
				modelAndView.addObject("error", "Please enter valid credentials");
				deleteCookiesofBrowser(httpServletRequest, httpServletResponse);
			}
		} else if (httpServletRequest.getRequestURL().toString().endsWith(Constants_URLPath.LOGOUT)) {
	...
	@RequestMapping(value = Constants_URLPath.GENERATE_CAPTCHA, method = RequestMethod.POST)
	public @ResponseBody Map<String,String> captcha(HttpServletRequest request) {

		logger.info("RedirectController begining of captcha");
		return captchaUtil.genarateCaptcha(request);
	}
}
