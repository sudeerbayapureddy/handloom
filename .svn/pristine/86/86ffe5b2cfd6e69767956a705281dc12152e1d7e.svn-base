...
public class StakeHolderDAOImpl implements StakeHolderDAO{
	...
	public List<YarnIndent> getIndentList(Integer shId) {
		// TODO Auto-generated method stub
		Criteria loomDetails= sessionFactory.getCurrentSession().createCriteria(YarnIndent.class,"indentdetails");
		/*
		loomDetails.createAlias("indentdetails.hsStakeHolders", "hsStakeHolders",Criteria.LEFT_JOIN);
		...
		loomDetails.add(Restrictions.eq("indentdetails.yarnSHId", shId));	
	...
	@Override
	public List<LoomDetails> getYarnType(List<BigInteger> barCodes) {
		// TODO Auto-generated method stub
	...
	public String saveAncillaryDetails(List<AncillaryWorkerDetail> ancillaryWorkersList) {
		try
		{
			for(AncillaryWorkerDetail wokerDetails:ancillaryWorkersList){
				...
				workersMapping.setAwmLoomId(wokerDetails.getLoomId());
				workersMapping.setCreated_By(wokerDetails.getCreated_By());
				workersMapping.setCreated_Date(new Date());
	...
	public Integer getUserIdByUserName(String userName) {
		return (Integer)hibernateUtil.createEntityCriteria(User.class).add(Restrictions.eq("userName", userName)).setProjection(Projections.property("userId")).uniqueResult();
	}

	@Override
	public User checkRegistrationByUserId(String uname) {
		User user=(User) hibernateUtil.createEntityCriteriaByAlias(User.class, "user").add(Restrictions.eq("userName", uname)).uniqueResult();
		user.getStakeHolder();
		return user;
	}
	
	@Override
	public synchronized  String getMaxNoByColumn(Class<?> entityClass, String columnName) {
		return hibernateUtil.getMaxValueByColumn(entityClass, columnName);
	}
